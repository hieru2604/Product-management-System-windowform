...
namespace LaptopDemo
{
    public partial class StockIn : Form
    {
        ...
        public void LoadStockIn() 
        {
            ...
            cm = new SqlCommand("SELECT * FROM vwStockIn WHERE status LIKE 'Pending' and refno LIKE '" + txtRefNo.Text + "'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read()) 
            {
                ...
                if (dgvStockIn.Rows.Count > 0) 
                {
                    if (MessageBox.Show("Are you sure you want to save this records?", stitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        for (int i = 0; i < dgvStockIn.Rows.Count; i++)
                        {
                            ...
                            cn.Open();
                            cm = new SqlCommand("UPDATE tbProducts SET qty = qty +" + int.Parse(dgvStockIn.Rows[i].Cells[5].Value.ToString()) + "WHERE pcode LIKE '" + dgvStockIn.Rows[i].Cells[3].Value.ToString() + "'", cn);
                            cm.ExecuteNonQuery();
                            cn.Close();
        ...
        private void btnLoadRecord_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cm = new SqlCommand("SELECT * FROM vwStockIn WHERE sdate  between '" + dtFrom.Value + "' and '" + dtTo.Value + "' AND status LIKE 'DONE' ORDER BY sdate DESC", cn);
                cn.Open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
