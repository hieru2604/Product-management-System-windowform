...
namespace LaptopDemo
{
    public partial class Product : Form
    {
        ...
        public void LoadProduct()   //get product list from database
        {
            int i = 0;
            dgvProduct.Rows.Clear();
            cm = new SqlCommand("SELECT p.pcode, p.pname, p.pdesc, b.brand, c.category, p.price, p.reorder FROM tbProducts AS p INNER JOIN tbBrand AS b ON b.id = p.bid INNER JOIN tbCategory as c on c.id = p.cid ORDER BY pcode ASC", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void btnAdd_Click(object sender, EventArgs e)       //show product module form
        {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)            //product dgv button interacts
        {
            ...
            if (colName == "Edit")
            {
                ...
                product.txtPname.Text = dgvProduct.Rows[e.RowIndex].Cells[2].Value.ToString();
            ...
            else if(colName == "Delete")
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbProducts WHERE pcode LIKE '" + dgvProduct[1, e.RowIndex].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
